...
fn select_steam_target(app_id: u32, process_name: Option<&str>) -> Result<ProcessInfo> {
    ...
    // AppID alone includes Wine services and launch wrappers, so require the
    // actual guest PE path to live inside the selected game's install tree
    ...
    match candidates.len() {
        ...
        count => {
            ...
            Err(Error::InvalidInput(format!(
                "{count} trusted game processes matched Steam AppID {app_id}: {identities}; add \
                 --process or use --pid"
            )))
        }
    }
